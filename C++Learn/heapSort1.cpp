 ...
 */
vector<int> HeapSort(vector<int> input, int k) {
    int len=input.size();
    if(len<=0||k<=0||k>len)
        return vector<int>();

    vector<int> res(input.begin(),input.begin()+k);//前K个数入容器
    make_heap(res.begin(),res.end(),greater<int>());  //再入堆   默认为最大堆，最小堆使用less<int>() make_heap(res.begin(), res.end(), less<int>());
...
vector<int> HS(vector<int> input,int k)
{
    int len=input.size();
    if(len<=0||k<=0||k>len)
        return vector<int>();
